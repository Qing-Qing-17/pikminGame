# pikminGame

皮克敏主題的遊戲，細胞分裂與戲精酒吧。

由指揮官（主持人）投影或自用一台裝置，參加者用手機掃同一個網頁、輸入房間代碼加入。
兩種活動流程：

- **星攻略**：皮克敏迫降 → 偽裝洞穴
- **培訓**：皮克敏迫降 → 情報交換 → 偽裝洞穴

每個活動的第一頁是**設定**：先定好小隊數量，才產生六位數字的房間代碼與 **QR code**。
參加者用手機相機掃 QR code 就進到「輸入房間代碼」的畫面，代碼已經填好。
代碼與縮小的 QR code 之後會固定待在畫面右上角，晚到的人隨時掃得進來。

指揮官按下「進入故事」才開始說故事，故事一頁一句由指揮官控制節奏；
這段期間皮克敏的手機只顯示「待續…」，讓大家抬頭看投影而不是低頭看手機。

固定的皮克敏（玩家）入口，代碼要自己輸入，可以事先印在講義或投影片上：

<https://qing-qing-17.github.io/pikminGame/?join>

指揮官與皮克敏是兩個各自獨立的介面，不能互相切換——現場一台裝置只會是其中一種身分。

線上版：<https://qing-qing-17.github.io/pikminGame/>

> ⚠️ **請先實測一次**：Firebase 資料庫已設定完成，但開發環境的出口代理封鎖了
> 外部網域，我無法從那裡驗證正式環境。第一次部署後請開啟網站，確認狀態列顯示
> 「已連線同步」，並用另一支手機實際加入房間試一次。

## 執行方式

`index.html` 是原始碼，直接用瀏覽器打開就能跑（JSX 在瀏覽器裡即時編譯，
需要能連到 cdnjs 與 cdn.tailwindcss.com）。

正式活動請改用建置版：

```bash
npm install
npm run build      # 產出 dist/index.html
```

`dist/index.html` 把 JSX 預先編譯、Tailwind 產成靜態 CSS、React 直接內嵌，
不依賴任何 CDN，載入也快得多（可互動時間約 2.1s → 1.4s）。部署時上傳這一個檔案即可。
唯一的外部資源是 Google Fonts；被擋掉時會退回系統字型，不影響功能。

## 發佈到 GitHub Pages

`.github/workflows/deploy.yml` 會在推上 `main` 時自動建置並發佈 `dist/`，
網址是 `https://<帳號>.github.io/pikminGame/`。

流程裡的 `actions/configure-pages` 帶了 `enablement: true`，Pages 沒開啟時它會直接
用 API 開起來，所以**不需要手動到 Settings 設定**。若那一步失敗（例如組織政策擋住），
再到 **Settings → Pages → Build and deployment → Source** 選 **GitHub Actions** 即可。

發佈的是建置版，不是根目錄的原始碼——原始碼要在瀏覽器裡即時編譯 JSX，
現場用手機開會明顯變慢，而且一旦連不到 cdnjs 就整頁打不開。

## 設定 Firebase

多人同步用 Firebase Realtime Database。用 REST API，**不需要 API key，也不需要
安裝任何 SDK**——只要一個資料庫網址。

目前使用的資料庫是
`https://pikmingame-3d82e-default-rtdb.asia-southeast1.firebasedatabase.app`，
已經填在 `index.html` 開頭的 `FIREBASE_DB_URL`。這個網址會出現在網頁原始碼裡，
是 Firebase 用戶端設定的正常用法。

要換成別的專案時：

1. 到 <https://console.firebase.google.com> 建立專案（不需要開 Google Analytics）
2. 左側 **建構 → Realtime Database → 建立資料庫**
3. 地區選 **asia-southeast1**（新加坡，離台灣最近）
4. 安全性規則先選**測試模式**
5. 把畫面上方顯示的網址填進 `FIREBASE_DB_URL`
6. 推上 `main`，網站會自動重新建置與部署

**測試模式的規則有效期是 30 天**，到期後會全部拒絕。活動前請確認還在期限內，
或改成明確的規則。以下規則允許任何人讀寫房間，但擋掉其他路徑：

```json
{ "rules": { "rooms": { "$room": { ".read": true, ".write": true } } } }
```

## 多人同步

狀態存在 Firebase Realtime Database，房間代碼是**六位純數字**（現場念數字比念英數快，
也不會有 0/O、1/I/L 看錯的問題）。每個房間長這樣：

```
rooms/<代碼>/session                      指揮官擁有的流程狀態
rooms/<代碼>/cards                        任務卡
rooms/<代碼>/taken/<小隊>/<號碼>            = 認領者的玩家 id
rooms/<代碼>/profiles/<玩家 id>            = 情報（不含答案）
rooms/<代碼>/guesses/<對象>/<判讀者>        = { attempts, pick, result }
```

這個結構讓幾類問題從根本消失，而不是靠重試補救：

- **指揮官用 `PATCH` 只更新流程欄位**，玩家的資料在別的路徑，不可能被蓋掉。
- **每位玩家只寫自己的路徑**（`profiles/<我>`、`guesses/<對象>/<我>`），零競爭。
- **搶情境牌用 `ETag` 條件寫入**（`if-match`）：第二個人會收到 412，換下一個號碼再試。
  兩人拿到同一號在結構上不可能發生。
- **用 SSE 即時推播取代輪詢**：靜置時完全不發請求，指揮官變更後約 0.3 秒就同步到玩家。

其餘仍然保留的機制：

- 同一台裝置的寫入串成佇列，避免亂序抵達讓關卡倒退。
- 指揮官的房間代碼記在本機，重新整理或不小心關掉分頁後回到同一個房間，
  已經發出去的連結與 QR code 仍然有效。
- 倒數以指揮官的時鐘為共同基準（實測兩台裝置時鐘差 5 分鐘時，倒數相差 0 秒）。
- 指揮官保留本機備份，房間資料被清空時自動還原。
- 未設定或連不上時退回單機模式，並明確告知原因，不對死掉的端點空轉。

**已知限制**：測試模式的規則等於公開讀寫，參與者可透過開發者工具竄改狀態。
真正的權限控管需要登入機制。假情報的答案不會上傳，只留在本人的裝置上並由該裝置
判定對錯。

## 現場操作

- **流程**：選活動 → 設定小隊數並發出代碼／QR code → 進入故事 → 各個關卡。
  房間代碼與小 QR code 從產生後就固定在右上角，不會擋住投影內容。
- **翻頁**：故事一頁一句，指揮官按「下一頁／上一頁」控制節奏，字級在投影與手機上都放大過。
  故事進行時皮克敏的手機顯示「待續…」。
- **集合人數**（皮克敏迫降／細胞分裂）：畫面只顯示一個大數字，那就是這一輪大細胞的人數。
  可切「手動指定」（＋／−）或「自動隨機」——自動模式每 1～2 分鐘自己換一次，
  畫面上會顯示距離下次還有幾秒，也可以按「現在就換一個」立刻換。
- **音樂**：由指揮官自己用外部播放器放，App 內不內建。
- **倒數計時器**：是標題列的獨立小工具，不佔關卡畫面。點開後可設定與開始，
  按「返回」關掉，按「縮小視窗」會縮成右下角的小按鈕——計時繼續跑，也不擋住底下的操作。
- **表演方式**（偽裝洞穴）：指揮官可切「動作／表情／聲音」，玩家端會同步顯示對應的提示。
- **猜題者**：每一小隊各抽出一位猜題者，該號碼的人手機上會顯示猜題者身分，
  表演者只看得到自己那一題，看不到完整情境清單。

### 皮克敏迫降（細胞分裂）的規則

規則寫在序章故事裡，由指揮官一頁一句帶完，不需要另外口頭說明：

- 畫面上的數字就是這一輪「大細胞」的人數；看到數字就找人**並排站成一列**，人數要剛剛好。
- 沒擠進任何大細胞的人是「小細胞」，必須在外面奔波，先找其他還沒合體的人湊成新的一組。
- 湊不到人就去撞已完成的大細胞：**從左邊插進去，最右邊的人被擠出來；從右邊插進去，換最左邊的人。**
- 大細胞人數固定，所以每擠進一個就一定有一個人被推出去。
- 剛被擠出來的人有一小段空檔不會被病毒抓到，可以再去撞下一顆；停在原地不動的一定被抓。
- 指揮官隨時改變數字，數字一變全場的大細胞都要重組。時間到還沒待在完整大細胞裡的人出局。

故事可以用「編輯故事」改寫，一行就是一頁。

## 測試

見 [`tests/README.md`](tests/README.md)。用 Playwright 開多個獨立的瀏覽器 context
模擬多台裝置並模擬網路延遲，涵蓋多人同時操作、離線降級、時鐘偏差、無障礙等情境。
